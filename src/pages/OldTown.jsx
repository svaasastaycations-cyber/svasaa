...
import { MapPin, BedDouble, PartyPopper, Tv, Wifi, Shield, Zap, Utensils, Waves, Trophy, Flame, Gamepad2, Bath, Users, ChevronLeft, ChevronRight } from 'lucide-react';
      ...
      <div className="container mx-auto px-6 max-w-7xl">
        ...
        <section className="pt-20 pb-12 border-b border-white/10 text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 max-w-5xl mx-auto">
            <div className="flex flex-col items-center group">
              ...
              <span className="text-xl md:text-2xl font-bold text-svaasa-brand leading-tight">Just 25 mins from ORR</span>
            </div>
            <div className="flex flex-col items-center group">
              <div className="w-20 h-20 rounded-full bg-white/5 border border-white/10 flex items-center justify-center mb-6 group-hover:bg-svaasa-brand group-hover:text-black transition-all duration-500">
                <BedDouble size={32} strokeWidth={1.5} />
              </div>
              <span className="text-xl md:text-2xl font-bold text-svaasa-brand leading-tight">6 Vintage Themed Luxury Rooms</span>
            </div>
            <div className="flex flex-col items-center group">
              <div className="w-20 h-20 rounded-full bg-white/5 border border-white/10 flex items-center justify-center mb-6 group-hover:bg-svaasa-brand group-hover:text-black transition-all duration-500">
                <Users size={32} strokeWidth={1.5} />
              </div>
              <span className="text-xl md:text-2xl font-bold text-svaasa-brand leading-tight">Upto 25 guests</span>
            </div>
          </div>
        </section>
        ...
        <section className="pt-20 pb-12 border-b border-white/10 text-center">
          <div className="max-w-5xl mx-auto">
            ...
            <div className="text-lg md:text-2xl text-svaasa-gray font-header font-light leading-relaxed space-y-6 md:space-y-10 italic">
              <p>
                The Old Town is a <strong className="text-svaasa-brand">vintage-themed luxury villa just 25 min from ORR</strong>—close to the city, yet secluded for complete privacy. Surrounded by lush greenery, it's a sanctuary designed for those who appreciate the finer things in life.
              </p>
              <p>
                Our <strong className="text-svaasa-brand">6 handcrafted rooms</strong> blend classic charm with modern amenities, making it the perfect choice for family gatherings, intimate celebrations, or a peaceful corporate retreat.
              </p>
              <p>
                Comfortably hosting <strong className="text-svaasa-brand">up to 25 guests</strong>, it’s ideal for parties, group stays, and special occasions. With an <strong className="text-svaasa-brand">event space for up to 300 guests</strong>, every celebration feels elevated. Come for the stay, stay for the vibe—because once you’re here, you won’t want to leave.
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="pt-20 pb-12 border-b border-white/10 text-center">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-12 max-w-6xl mx-auto">
            {amenities.map((item, idx) => (
              <div key={idx} className="flex items-center space-x-4 md:space-x-6 bg-white/5 p-4 md:p-8 rounded-3xl border border-white/10 transition-all duration-500 group">
                <div className="text-svaasa-brand shrink-0 group-hover:scale-110 transition-transform duration-300">{item.icon}</div>
                <span className="text-[10px] md:text-sm font-black tracking-widest uppercase text-white/80 text-left leading-tight">{item.name}</span>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="pt-20 pb-12 border-b border-white/10 text-center">
            ...
            <table className="w-full text-center border-collapse min-w-[500px] md:min-w-0">
              <thead>
                <tr className="border-b border-white/5">
                  <th className="p-6 md:p-8 text-svaasa-brand font-header font-bold text-sm md:text-lg tracking-wider text-left">Guest Capacity</th>
                  <th className="p-6 md:p-8 text-svaasa-brand font-header font-bold text-sm md:text-lg tracking-wider text-right">Weekdays</th>
                  <th className="p-6 md:p-8 text-svaasa-brand font-header font-bold text-sm md:text-lg tracking-wider text-right">Weekends</th>
                </tr>
              </thead>
              <tbody className="text-white font-header">
                <tr className="border-b border-white/5 hover:bg-white/5 transition-colors">
                  <td className="p-6 md:p-8 text-left text-lg md:text-2xl font-light">Upto 15 Guests</td>
                  <td className="p-6 md:p-8 text-right text-lg md:text-2xl font-light">₹ 24,000/-</td>
                  <td className="p-6 md:p-8 text-right text-lg md:text-2xl font-light">₹ 26,000/-</td>
                </tr>
                <tr className="hover:bg-white/5 transition-colors">
                  <td className="p-6 md:p-8 text-left text-lg md:text-2xl font-light">From 15-30 Guests</td>
                  <td className="p-6 md:p-8 text-right text-lg md:text-2xl font-light">₹ 27,000/-</td>
                  <td className="p-6 md:p-8 text-right text-lg md:text-2xl font-light">₹ 30,000/-</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <p className="text-sm mt-12 text-white/30 font-header tracking-widest uppercase italic font-bold">
            *Prices are inclusive of taxes and subject to change during long weekends or public holidays.
          </p>
        </section>
        {/* 7. Gallery Section */}
            ...
            <div className="mt-4 flex items-center justify-between max-w-[300px] mx-auto px-4">
              ...
              <div className="flex-1 mx-6 h-[3px] bg-white/30 relative rounded-full overflow-hidden">
                <div 
                  ...
                  style={{ width: `${Math.max(20, 100 / 3)}%`, left: `${(scrollProgress * (2/3))}%` }}
                />
              </div>
